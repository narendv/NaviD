...
from recon_datavis.gps.conversions import latlong_to_utm, utm_to_latlong


class GPSPlotter(object):

    def __init__(self,
                 nw_latlong=(37.915265, -122.334964),
                 se_latlong=(37.914700, -122.334358),
                 zoom=19,
                 satellite_img_fname="current.png",
    ...
    def plot_latlong_path(self, ax, latlong, latlong_ar, blit=True, color_path='b', color='r'):
        x, y = self.latlong_to_coordinate(latlong)
        coord_ar = np.array([self.latlong_to_coordinate(latlong_ar[i]) for i in range(len(latlong_ar))])
        ax.cla()

        imshow = ax.imshow(np.flipud(self._img), origin='lower')
        # curr_point = ax.plot(x, y, color=color, marker='x', ms=5.)
        path = ax.plot(coord_ar[:, 0], coord_ar[:, 1], color=color_path, lw=3.)

            # if blit:
            #     ax.draw_artist(ax.patch)
            #     ax.draw_artist(imshow)
            #     ax.draw_artist(curr_point)
            #     ax.draw_artist(path)


    def plot_latlong(self, ax, latlong, blit=True, color='r'):
        return self.plot_utm(ax, latlong_to_utm(latlong), blit=blit, color=color)
    ...
    def plot_utm(self, ax, utm,
                                     blit=True, color='r', arrow_length=15, arrow_head_width=10):
        x, y = self.utm_to_coordinate(utm)

        if ax not in self._plt_latlong_and_compass_bearing_dict:
            imshow = ax.imshow(np.flipud(self._img), origin='lower')
            arrow = ax.arrow(x, y, 0, 0, color=color, head_width=arrow_head_width)
            ax.get_xaxis().set_visible(False)
            ax.get_yaxis().set_visible(False)
            self._plt_latlong_and_compass_bearing_dict[ax] = (imshow, arrow)
        else:
